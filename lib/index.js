...
export class Collection {
  ...
  async getMany(options = {}) {
    ...
    if (params.page && params.skip) throw new Error('Cannot use page and skip together')

    const queryString = Object.keys(params).map(key => `${key}=${params[key]}`).join('&')
    return await this.store.flexdb.axiosCall('get', `/collections/${this.name}?${queryString}`, null, this.store.id)
  }
